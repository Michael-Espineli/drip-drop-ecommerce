...
const customerIsActive = (customer = {}) => customer.active === true;

const companyUserOptionValue = (user = {}) => String(user.userId || user.id || user.docId || "").trim();

const companyUserLookupIds = (user = {}) => new Set([
  user.userId,
  user.id,
  user.docId,
  user.uid,
...
const companyUserLookupNames = (user = {}) => new Set([
  getCompanyUserDisplayName(user, ""),
  user.userName,
  user.displayName,
...
const recordMatchesCompanyUser = (record = {}, companyUser = null) => {
  if (!companyUser) return false;

  const lookupIds = companyUserLookupIds(companyUser);
  const lookupNames = companyUserLookupNames(companyUser);
  const recordIds = [
    record.techId,
    record.userId,
    record.technicianId,
  ...
  return recordIds.some((id) => lookupIds.has(id)) || recordNames.some((name) => lookupNames.has(name));
};
...
const CustomerTagManager = () => {
  ...
  const regionVisibleCustomers = useMemo(
  ...
  const visibleCustomers = useMemo(() => {
    if (statusFilter === "all") return regionVisibleCustomers;
    if (statusFilter === "inactive") return regionVisibleCustomers.filter((customer) => !customerIsActive(customer));
    return regionVisibleCustomers.filter(customerIsActive);
  }, [regionVisibleCustomers, statusFilter]);
  const visibleCustomerIds = useMemo(
    () => new Set(visibleCustomers.map((customer) => customer.id)),
    [visibleCustomers]
  );
  const visibleCustomerSummary = useMemo(() => {
    const active = regionVisibleCustomers.filter(customerIsActive).length;
    return {
      active,
      inactive: regionVisibleCustomers.length - active,
    };
  }, [regionVisibleCustomers]);
  ...
  const selectedTechnician = useMemo(
    () => companyUsers.find((companyUser) => companyUserOptionValue(companyUser) === technicianId) || null,
    [companyUsers, technicianId]
  );
  const technicianRoutes = useMemo(
    () => selectedTechnician
      ? routes.filter((route) => recordMatchesCompanyUser(route, selectedTechnician))
      : [],
    [routes, selectedTechnician]
  );
  ...
  const selectedTechnicianCustomerIds = useMemo(() => {
    ...
    recurringStops.forEach((stop) => {
      if (!recordMatchesCompanyUser(stop, selectedTechnician)) return;
      if (stop.customerId && visibleCustomerIds.has(stop.customerId)) ids.add(stop.customerId);
    });

    return [...ids];
  }, [recurringStops, recurringStopsById, selectedTechnician, technicianRoutes, visibleCustomerIds]);
  ...
  const loadTagData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [customerSnap, locationSnap, recurringStopSnap, routeSnap, companyUserSnap] = await Promise.all([
      ...
      setCompanyUsers(sortCompanyUsersByName(
        companyUserSnap.docs.map((userDoc) => ({ id: userDoc.id, ...userDoc.data() }))
      ));
    } catch (error) {
  ...
  const assignActiveTagToTechnicianAccess = async () => {
    if (!requirePermission("264", "update company user tag access")) return;
    if (!activeTag) {
      toast.error("Choose or enter a tag first.");
      return;
    }
    ...
    const assignedTags = normalizeCustomerTags([...getCustomerTagAccessList(selectedTechnician), activeTag]);
    const payload = {
      fullCustomerRegionAccess: false,
      fullRegionalAccess: false,
      regionalAccessMode: "limited",
      customerRegionAccessMode: "limited",
    ...
    try {
      ...
      await updateCompanyUserAccess({
        ...
        companyUserId: selectedTechnician.id,
        updates: payload,
        idToken,
      });

      setCompanyUsers((currentUsers) => currentUsers.map((companyUser) => {
        if (companyUser.id !== selectedTechnician.id) return companyUser;
        return {
          ...companyUser,
          fullCustomerRegionAccess: false,
          fullRegionalAccess: false,
      ...
      toast.success(`Assigned ${activeTag} to ${getCompanyUserDisplayName(selectedTechnician)}.`);
    } catch (error) {
      console.error("Error assigning tag access to technician:", error);
      toast.error("Failed to assign tag access.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="w-full space-y-5">
        ...
        <section className="grid gap-5 xl:grid-cols-3">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-base font-semibold text-slate-950">Tag By Technician</h2>
                <p className="mt-1 text-sm text-slate-500">Uses recurring routes and assigned recurring stops.</p>
              </div>
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">
                {selectedTechnicianCustomerIds.length} customer{selectedTechnicianCustomerIds.length === 1 ? "" : "s"}
              </span>
            </div>
            <select
              value={technicianId}
            ...
            >
              ...
              {companyUsers.map((companyUser) => {
                const optionValue = companyUserOptionValue(companyUser);
                return (
                  <option key={optionValue || companyUser.id} value={optionValue}>
            ...
            <p className="mt-2 text-xs font-medium text-slate-500">
              {technicianRoutes.length} recurring route{technicianRoutes.length === 1 ? "" : "s"} found.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => addTagToCustomers(selectedTechnicianCustomerIds)}
                disabled={isSaving || selectedTechnicianCustomerIds.length === 0}
                className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Add Tag to Technician
              </button>
              <button
                type="button"
                onClick={() => setSelectedCustomerIds(selectedTechnicianCustomerIds)}
                disabled={selectedTechnicianCustomerIds.length === 0}
                className="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Select Technician Customers
              </button>
              <button
                type="button"
                onClick={assignActiveTagToTechnicianAccess}
                disabled={isSaving || !activeTag || !technicianId}
                className="rounded-md border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 shadow-sm transition hover:bg-blue-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Assign Tag Access
              </button>
            </div>
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  ...
                  {" "}
                  {visibleCustomerSummary.active} active, {visibleCustomerSummary.inactive} inactive.
                </p>
              </div>
